...
const DropdownQuestion = ({
  ...
  useEffect(() => {
    ...
    if (
      currentUserAnswer.answerText &&
      Array.isArray(currentUserAnswer.answerText)
    ) {
      ...
      {processedData.type === "paragraph" ? (
        <>
          {Object.entries(processedData.answers).map(([key, options]) => (
            <div key={key} className="dropdown-item flex w-full items-center">
              ...
              {error[key] && (
                <p className="mt-2 text-base text-red-500">{error[key]}</p>
              )}
            </div>
          ))}
        </>
        ...
        <div className="w-full space-y-6">
          {processedData.leftItems.map((leftItem, index) => (
            <div
              key={index}
              className="dropdown-item flex w-full items-center gap-6"
            >
              ...
              {error[leftItem] && (
                <p className="mt-2 text-base text-red-500">{error[leftItem]}</p>
              )}
            </div>
          ))}
        </div>
