...
const CreateReading = () => {
  ...
  const [fillBlankSentences, setFillBlankSentences] = useState([
    {
      ...
      options: [{ id: 1, label: 'A', value: 'fifty' }, { id: 2, label: 'B', value: 'sixty' }],
      correctOptionId: 1
    }
  ]);
  ...
  const addTfngSentence = () => setTfngSentences([...tfngSentences, { id: getNextId(tfngSentences), text: '', correctAnswer: null }]);
  const removeTfngSentence = (id) => setTfngSentences(tfngSentences.filter(s => s.id !== id));
  const setTfngAnswer = (id, value) => {
    const updated = tfngSentences.map(s => s.id === id ? {...s, correctAnswer: value} : s);
    ...
    <div className="flex flex-col gap-6 pb-10">
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col gap-4">
          <div><label className="font-medium text-gray-700">Name <span className="text-red-500">*</span></label><Input placeholder="e.g., Part 1: Reading Passage 1" size="large" /></div>
          <div><label className="font-medium text-gray-700">Instruction</label><TextArea rows={3} placeholder="e.g., Questions 1-5" /></div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
          <div><label className="font-medium text-gray-700">Question Name <span className="text-red-500">*</span></label><Input placeholder="e.g., Q1-Reading" size="large" /></div>
          <div>
        ...
        <div className="mb-6">
             ...
             <TextArea rows={4} placeholder="Enter the question text or the reading passage segment students will see..." />
        </div>
        ...
        {questionType === 'true-false' && (
          <div className="flex flex-col gap-6">
             <Button icon={<PlusOutlined />} size="large" className="w-40 border-blue-900 text-blue-900 font-medium" onClick={addTfngSentence}>Add Statement</Button>
             {tfngSentences.map((item, idx) => (
                 <div key={item.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                     <div className="flex justify-between items-start mb-2">
                         <span className="font-bold text-gray-700">Statement {idx + 1}</span>
                         <DeleteOutlined className="text-red-500 cursor-pointer" onClick={() => removeTfngSentence(item.id)}/>
                     </div>
                     <Input className="mb-4" size="large" placeholder="e.g., The population increased significantly." 
                        value={item.text} 
                        onChange={(e) => {
                            const updated = tfngSentences.map(s => s.id === item.id ? {...s, text: e.target.value} : s);
                            setTfngSentences(updated);
                        }}
                     />
                     <div className="flex items-center gap-4">
                         <span className="text-sm font-semibold text-gray-600">Correct Answer:</span>
                         <Radio.Group value={item.correctAnswer} onChange={(e) => setTfngAnswer(item.id, e.target.value)}>
        ...
        {questionType === 'matching' && (
          <div>
            <div className="grid grid-cols-2 gap-8 mb-6">
              <div>
                <h4 className="font-bold text-blue-900 mb-2">Questions / Headings</h4>
                <div className="flex flex-col gap-3">{matchingData.contents.map((item) => (<div key={item.id} className="flex items-center gap-2"><div className="text-blue-900 font-bold w-4">{item.label}</div><Input placeholder="Question text..." /></div>))}<Button type="dashed" block icon={<PlusOutlined />} onClick={addMatchingContent} className="mt-2">Add more</Button></div>
              </div>
              <div>
                <h4 className="font-bold text-green-600 mb-2">Paragraphs / Options</h4>
                <div className="flex flex-col gap-3">{matchingData.options.map((item) => (<div key={item.id} className="flex items-center gap-2"><div className="text-green-600 font-bold w-4 rounded-full border border-green-600 flex items-center justify-center h-6 text-xs">{item.label}</div><Input placeholder="Paragraph A..." /></div>))}<Button type="dashed" block icon={<PlusOutlined />} onClick={addMatchingOption} className="mt-2">Add more option</Button></div>
              </div>
            </div>
