...
const TopicListPage = () => {
  ...
  const handleBulkExport = () => {
    const selected = getSelectedTopics();
    const csvContent = [
      ['Topic Name', 'Status', 'Creator', 'Created At', 'Updated At'].join(','),
      ...selected.map((t) =>
        [
          `"${t.Name || ''}"`,
          `"${t.Status || ''}"`,
    ...
    return (
      <div className='fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.15)] border border-gray-200 px-5 py-3 flex items-center gap-4 animate-slide-up'>
        ...
        <Space size='small'>
          ...
          <Button icon={<CopyOutlined />} onClick={handleBulkClone}>Clone</Button>
          <Button icon={<ExportOutlined />} onClick={handleBulkExport}>Export</Button>
          <Button danger icon={<DeleteOutlined />} onClick={handleBulkDelete}>Delete</Button>
        </Space>
        <div className='w-px h-6 bg-gray-200' />
        <Button
          type='text'
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Tag color={cfg.antColor} className='font-semibold px-3 py-1 rounded-full border-none !m-0'>
        ...
        return (
          <div className="flex items-center justify-center gap-3">
            ...
            <button
              ...
              className={`cursor-pointer border-none bg-transparent transition-all hover:opacity-70 ${!canEdit ? 'opacity-40 cursor-not-allowed' : ''}`}
              title="Edit Topic"
              disabled={!canEdit}
            >
            ...
            <button
              ...
              className={`cursor-pointer border-none bg-transparent transition-all hover:opacity-70 ${!canArchive ? 'opacity-40 cursor-not-allowed' : ''}`}
              title="Archive Topic"
              disabled={!canArchive}
            >
