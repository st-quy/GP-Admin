...
const CreateExamPage = () => {
  ...
  const handlePreviewExam = () => {
    ...
    const skills = skillOrder.map((skillName) => {
      ...
      if (!found || !found.section)
        return { ID: null, Name: skillName, Parts: [] };
      return {
  ...
  const onNameChange = (e) => {
    ...
    if (/[^a-zA-Z0-9\s]/.test(value)) {
      message.warning(
        'Special characters and emojis are not allowed in exam name.',
      );
  ...
  const renderSelectedSectionUI = () => {
    ...
    const questionCount = (selectedSection.Parts || []).reduce(
        ...
        >
          ...
          <div className='space-y-4'>
            {(selectedSection.Parts || []).map((part) => (
              <div
                key={part.ID}
                className='p-4 border-[1px] border-[#E5E7EB] rounded-lg bg-white shadow-sm'
              >
                <Text className='font-bold text-[#111827] block mb-2'>
                  {part.Content}
                </Text>
                {part.SubContent && (
                ...
                {!(
                  ...
                  <div className='mt-2 space-y-2'>
                    {(part.Questions || []).map((q, index) => (
                      <div
                        key={q.ID}
                        className='flex items-center gap-3 p-2 rounded bg-[#F9FAFB] border border-[#E5E7EB]'
                      >
                        <div className='w-6 h-6 rounded-full bg-[#003087] text-white flex items-center justify-center text-[12px] font-bold shrink-0'>
                          {index + 1}
                        </div>
                        <Text className='text-[13px] flex-1 line-clamp-1'>
                          {q.Content}
                        </Text>
                      </div>
                    ))}
      ...
      <div className='figma-content-wrapper'>
          ...
          <div className='py-8'>
            ...
            <Form form={form} layout='vertical'>
              ...
              >
                ...
                <div className='p-8'>{renderSelectedSectionUI()}</div>
              </Card>
              ...
              <div className='flex justify-between items-center mt-10 pb-10'>
                ...
                <Space size='middle'>
                  ...
                  {isViewMode &&
                    isAdmin &&
                    (topicData?.data?.Status === 'submited' ||
                      topicData?.Status === 'submited') && (
                      <>
                        <Button
                          type='primary'
                          style={{
        ...
        >
          <p className='text-[#4B5563] text-[15px] leading-[22px]'>
            You have unsaved changes in your exam structure. Would you like to
            save your work as a draft before leaving this page?
          </p>
        </Modal>
      </div>
    </div>
